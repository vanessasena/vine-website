...
export default function RegisterVisitorClient() {
	...
	const [phoneToConfirm, setPhoneToConfirm] = useState<string>('');
	...
	const handleSubmit = async (event: React.FormEvent) => {
		...
		const phoneToConfirmValue = visitor.phone.trim();
		setPhoneToConfirm(phoneToConfirmValue);
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
					<div className="bg-white rounded-lg shadow-xl max-w-sm w-full mx-4 p-6">
						<h3 className="text-lg font-semibold text-gray-900 mb-4">
							{t('confirmPhonePrompt', { phone: phoneToConfirm })}
						</h3>
						<div className="flex gap-3 justify-end">
							<button
								type="button"
								onClick={() => handlePhoneConfirmation(false)}
								className="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 font-medium shadow-sm transition hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2"
							>
								{t('noButton')}
							</button>
							<button
								...
								className="px-4 py-2 rounded-lg bg-primary-600 text-white font-medium shadow-sm transition hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2"
							>
								{t('yesButton')}
							</button>
						</div>
